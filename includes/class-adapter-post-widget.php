...
class Adapter_Post_Widget extends WP_Widget {
  ...
  function form( $instance ) {
	...
	<?php else : _e( 'No posts on your site. Please write one.' , 'adapter-post-preview' );
	endif;
       ?>
      </p>
  <?php 
  ...
  private function get_single_post_preview_without_carousel( $post_id ) {
    global $post;
    if ( $post->ID == $post_id ) {
      return ""; // the post is already showing on the page, so no need for a widget preview of it
    }
